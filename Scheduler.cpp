...
void Scheduler::Init() {
    ...
    // DVFS: keep all machines on but start at a lower P-state
    for (unsigned i = 0; i < total_machines; i++) {
...
void Scheduler::NewTask(Time_t now, TaskId_t task_id) {
    ...
    // DVFS: round-robin across compatible machines, then adjust P-state
    {
        MachineId_t target = (MachineId_t)-1;
        for (unsigned n = 0; n < total_machines; n++) {
            unsigned idx = (rr_index + n) % total_machines;
            MachineInfo_t info = Machine_GetInfo(MachineId_t(idx));
            if (info.cpu != required_cpu) continue;
            if (info.s_state != S0) continue;
            target = MachineId_t(idx);
        ...
        if (target != (MachineId_t)-1) {
            ...
            // scale up P-state if machine is getting busy
            MachineInfo_t info = Machine_GetInfo(target);
            double util = (double)info.active_tasks / info.num_cpus;
            if (util > 0.7)
                Machine_SetCorePerformance(target, 0, P0);
            else if (util > 0.4)
                Machine_SetCorePerformance(target, 0, P1);
            else
                Machine_SetCorePerformance(target, 0, P2);
        }
    }
...
void Scheduler::TaskComplete(Time_t now, TaskId_t task_id) {
    ...
    // scale down P-state on machines that got lighter
    for (unsigned i = 0; i < total_machines; i++) {
        MachineInfo_t info = Machine_GetInfo(MachineId_t(i));
        if (info.s_state != S0) continue;
...
void Scheduler::PeriodicCheck(Time_t now) {
    ...
    // periodically re-evaluate DVFS for all machines
    for (unsigned i = 0; i < total_machines; i++) {
        MachineInfo_t info = Machine_GetInfo(MachineId_t(i));
        if (info.s_state != S0) continue;
        double util = (info.num_cpus > 0) ? (double)info.active_tasks / info.num_cpus : 0.0;
